...
add_action('rest_api_init', function () {
    register_rest_route('qipalin/v1', 'weixin/getopenid', array('methods' => 'POST', 'callback' => 'getOpenid'));
});
            ...
            if (empty($access_array['errcode'])) {
                ...
                if ($errCode == 0) {
                    if (!username_exists($openid)) {
                        ...
                        $userdata = array('user_login' => $openid, 'user_nicename' => $unionId, 'display_name' => $avatarUrl, 'user_pass' => NULL);
...
<form method="post" action="options.php">
    <?php 
    settings_fields('weixinapp-group');
    ?>
    <?php 
    do_settings_sections('weixinapp-group');
    ?>
    <table class="form-table">
        <tr valign="top">
        <th scope="row">AppID</th>
        <td><input type="text" name="wf_appid" style="width:400px" value="<?php 
    echo esc_attr(get_option('wf_appid'));
    ?>" /></td>
        </tr>
         
        <tr valign="top">
        <th scope="row">AppSecret</th>
        <td><input type="text" name="wf_secret" style="width:400px" value="<?php 
    echo esc_attr(get_option('wf_secret'));
    ?>" /></td>
        </tr>         
    </table>
    <?php 
