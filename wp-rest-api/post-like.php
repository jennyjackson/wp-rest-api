...
add_action('rest_api_init', function () {
    register_rest_route('qipalin/v1', '/like', array('methods' => 'POST', 'callback' => 'postLike'));
});
...
function post_like_json($openid, $postid)
{
    ...
    if (empty($postmeta)) {
        if (add_post_meta($postid, $openid, 'like', true)) {
            $post_likes = (int) get_post_meta($postid, 'zm_like', true);
            update_post_meta($postid, 'zm_like', $post_likes + 1);
            $result["code"] = "success";
...
add_action('rest_api_init', function () {
    register_rest_route('qipalin/v1', '/islike', array('methods' => 'POST', 'callback' => 'getIsLike'));
});
